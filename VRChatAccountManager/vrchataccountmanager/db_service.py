...
def add_account(acc: Account) -> Account:
    """Add a new account and return the stored instance."""
...
def get_account(acc_id: int) -> Account | None:
    """Retrieve a single account by id."""
    with _ensure_engine() as session:
        return session.get(Account, acc_id)
...
def delete_account(acc_id: int) -> None:
    """Remove an account and its bindings from the database."""
    with _ensure_engine() as session:
        acc = session.get(Account, acc_id)
        if acc is None:
            return
        for binding in session.exec(select(Binding).where(Binding.account_id == acc_id)):
            session.delete(binding)
        session.delete(acc)
        session.commit()


def get_bindings_for_project(product: str) -> List[Binding]:
    """Return bindings for a given project."""
    with _ensure_engine() as session:
        statement = select(Binding).where(Binding.product_name == product)
        return list(session.exec(statement))
